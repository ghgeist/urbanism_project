...
python -m ruff check --no-cache --fix api services scripts tests
...
- Run relevant tests.
- Run Python lint (`python -m ruff check --no-cache api services scripts tests`) when Python files are changed; fix lint errors before completion.
